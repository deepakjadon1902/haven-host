...
const roomSchema = z.object({
  ...
  images: z
    .array(
      z.string().refine((value) => value.startsWith("data:image/") || /^https?:\/\//.test(value)),
    )
    .max(20)
    .default([]),
  cover_image: z
...
function fromDbToRoomWithExisting(
  data: z.infer<typeof roomSchema> & { id?: string },
  existing?: Room,
): Room {
  return {
    ...fromDbToRoom(data),
...
function handleAuthFallback(error: unknown) {
  if (!isAuthFailure(error)) throw error;
  clearAdminToken();
}

function hasEmbeddedImages(data: z.infer<typeof roomSchema> | Partial<z.infer<typeof roomSchema>>) {
  return (
    data.cover_image?.startsWith("data:image/") ||
    data.images?.some((image) => image.startsWith("data:image/"))
  );
}
...
export async function adminCreateRoom(input: unknown): Promise<Room> {
  ...
  ensureSeeded();
  const room = fromDbToRoomWithExisting({ ...validated });
...
export async function adminUpdateRoom(input: unknown): Promise<Room> {
  ...
  const nextRoom = fromDbToRoomWithExisting(
    { ...(nextDb as z.infer<typeof roomSchema>), id: existing.id },
    existing,
  );
