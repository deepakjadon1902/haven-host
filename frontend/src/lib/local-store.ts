...
import {
  MAIN_HOTEL,
  findHotel as findSeedHotel,
  hotels as seedHotels,
  mockRooms,
} from "@/data/hotels";

const HOTELS_KEY = "haven.hotels.v1";
...
const PARTNERS_KEY = "haven.partners.v1";
...
function normalizeRoom(r: Room): Room {
  const hotel = r.hotelId ? findHotel(r.hotelId) : MAIN_HOTEL;
  const pricePerNightCents = Number(
    r.pricePerNightCents ?? Math.round(Number(r.pricePerNight ?? 0) * 100),
  );
  return {
    ...r,
    hotelId: r.hotelId ?? MAIN_HOTEL.id,
    hotelName: r.hotelName ?? hotel?.name ?? MAIN_HOTEL.name,
    hotelCity: r.hotelCity ?? hotel?.city ?? MAIN_HOTEL.city,
    pricePerNightCents,
    pricePerNight: Math.round(pricePerNightCents / 100),
...
function seedHotelsList(): Hotel[] {
  return seedHotels;
}

function seedPartners(): PartnerAccount[] {
  return seedHotels.map((hotel, index) => ({
    id: `partner-${hotel.slug}`,
    hotelId: hotel.id,
    hotelName: hotel.name,
    ownerName: hotel.partnerName,
    email:
      index === 0
        ? "partner@havenhost.com"
        : `${hotel.slug.replace(/-/g, ".")}@partner.havenhost.com`,
    password: index === 0 ? "partner123" : "partner123",
...
function seedRooms(): Room[] {
  const merged = mockRooms.map(normalizeRoom);
...
export function ensureSeeded() {
  if (!isBrowser()) return;
  const existingHotels = readJson<Hotel[]>(HOTELS_KEY);
  if (!existingHotels || !Array.isArray(existingHotels) || existingHotels.length === 0) {
    writeJson(HOTELS_KEY, seedHotelsList());
  }

  const existingPartners = readJson<PartnerAccount[]>(PARTNERS_KEY);
  if (!existingPartners || !Array.isArray(existingPartners) || existingPartners.length === 0) {
    writeJson(PARTNERS_KEY, seedPartners());
  }
  ...
  if (!existing || !Array.isArray(existing) || existing.length === 0) {
    ...
    const bySlug = new Map(existing.map((room) => [room.slug, normalizeRoom(room)]));
    let changed = false;
    ...
    if (existing.some((room) => !room.hotelId) || changed) {
      writeJson(ROOMS_KEY, Array.from(bySlug.values()));
    }
  }
...
export function listHotels(): Hotel[] {
  const seeded = isBrowser()
    ? (readJson<Hotel[]>(HOTELS_KEY) ?? seedHotelsList())
    : seedHotelsList();
  return seeded;
}

export function listPublicHotels(): Hotel[] {
  const activeHotelIds = new Set(
    listPartners()
      .filter((p) => p.active)
      .map((p) => p.hotelId),
  );
  return listHotels().filter((hotel) => activeHotelIds.has(hotel.id));
}

export function findHotel(slugOrId: string): Hotel | undefined {
  return (
    listHotels().find((h) => h.slug === slugOrId || h.id === slugOrId) ?? findSeedHotel(slugOrId)
  );
}

export function upsertHotel(hotel: Hotel): Hotel {
  ensureSeeded();
  const hotels = listHotels();
  const existingIdx = hotels.findIndex((h) => h.id === hotel.id);
  const next = hotels.slice();
  if (existingIdx === -1) next.unshift(hotel);
  else next[existingIdx] = hotel;
  if (isBrowser()) writeJson(HOTELS_KEY, next);
  return hotel;
}

export function listPartners(): PartnerAccount[] {
  ensureSeeded();
  return readJson<PartnerAccount[]>(PARTNERS_KEY) ?? seedPartners();
}

export function isPartnerHotelActive(hotelId: string | null | undefined): boolean {
  if (!hotelId) return false;
  return listPartners().some((partner) => partner.hotelId === hotelId && partner.active);
}

export function upsertPartner(
  input: Omit<PartnerAccount, "id" | "createdAt"> & { id?: string },
): PartnerAccount {
  ensureSeeded();
  ...
  const account: PartnerAccount = {
    ...input,
    id: input.id || `partner-${Date.now()}`,
    email: normalizedEmail,
    createdAt: partners.find((p) => p.id === input.id)?.createdAt ?? new Date().toISOString(),
  };
  const existingIdx = partners.findIndex((p) => p.id === account.id || p.email === normalizedEmail);
  const next = partners.slice();
  if (existingIdx === -1) next.unshift(account);
  else next[existingIdx] = account;
  if (isBrowser()) writeJson(PARTNERS_KEY, next);
  return account;
}

export function findPartnerByCredentials(input: {
  email: string;
  password: string;
}): PartnerAccount | null {
  const email = input.email.trim().toLowerCase();
  return (
    listPartners().find((p) => p.active && p.email === email && p.password === input.password) ??
    null
  );
}

export function updatePartnerStatus(input: { partnerId: string; active: boolean }): PartnerAccount {
  ensureSeeded();
  const partners = listPartners();
  const index = partners.findIndex((partner) => partner.id === input.partnerId);
  if (index === -1) throw new Error("Partner not found");
  const nextPartner = { ...partners[index], active: input.active };
  const next = partners.slice();
  next[index] = nextPartner;
  if (isBrowser()) writeJson(PARTNERS_KEY, next);
  return nextPartner;
}
...
export function listPublicRooms(): Room[] {
  return listRooms({ activeOnly: true }).filter((room) => isPartnerHotelActive(room.hotelId));
}
